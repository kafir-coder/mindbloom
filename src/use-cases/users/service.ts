...
import { IUserRepository, IUserSvc } from '../../interfaces/user';
...
export class UserSvc implements IUserSvc {
  ...
  async createUser(dto: CreateUserDto): Promise<User> {
    ...
    if (
      dto.role == 'Psychologist' &&
      !pysCode.includes(dto.psycode as string)
    ) {
  ...
  async updateUser(user_id: string, user: Partial<User>): Promise<User> {
  ...
  async updateKid(kid_id: string, kid: Partial<User>) {
